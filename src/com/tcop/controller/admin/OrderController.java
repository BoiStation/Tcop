...
import com.tcop.entity.ReturnsItem;
import com.tcop.entity.Shipping;
import com.tcop.entity.ShippingItem;
...
public class OrderController extends BaseController {
	...
	public String shipping(Long orderId, Long shippingMethodId,
		...
		if (!IIIllIlI(shipping, new Class[0]))
			return "/admin/common/error";
		if (order.isExpired()
				|| order.getOrderStatus() != com.tcop.entity.Order.OrderStatus.confirmed)
			return "/admin/common/error";
		if (order.getShippingStatus() != com.tcop.entity.Order.ShippingStatus.unshipped
				&& order.getShippingStatus() != com.tcop.entity.Order.ShippingStatus.partialShipment)
			return "/admin/common/error";
		...
		if (order.isLocked(admin)) {
			...
			shipping.setSn(IIlIIlIl
					.generate(com.tcop.entity.Sn.Type.shipping));
	...
	public String returns(Long orderId, Long shippingMethodId,
		...
		if (order.isExpired()
				|| order.getOrderStatus() != com.tcop.entity.Order.OrderStatus.confirmed)
			return "/admin/common/error";
		if (order.getShippingStatus() != com.tcop.entity.Order.ShippingStatus.shipped
				&& order.getShippingStatus() != com.tcop.entity.Order.ShippingStatus.partialShipment
				&& order.getShippingStatus() != com.tcop.entity.Order.ShippingStatus.partialReturns)
			return "/admin/common/error";
		Admin admin = IIIlllIl.getCurrent();
		if (order.isLocked(admin)) {
			return "/admin/common/error";
		} else {
			returns.setSn(IIlIIlIl.generate(com.tcop.entity.Sn.Type.returns));
	...
	public String update(Order order, Long areaId, Long paymentMethodId,
		...
		if (order1 == null)
			return "/admin/common/error";
		if (order1.isExpired()
				|| order1.getOrderStatus() != com.tcop.entity.Order.OrderStatus.unconfirmed)
			return "/admin/common/error";
		Admin admin = IIIlllIl.getCurrent();
