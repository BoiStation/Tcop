...
import com.tcop.dao.ProductCategoryDao;
  ...
  implements ProductCategoryDao
{
  public List<ProductCategory> findRoots(Integer count)
  {
    ...
    TypedQuery localTypedQuery = this.entityManager.createQuery(str, ProductCategory.class).setFlushMode(FlushModeType.COMMIT);
    if (count != null) {
  ...
  public List<ProductCategory> findParents(ProductCategory productCategory, Integer count)
  {
    ...
    TypedQuery localTypedQuery = this.entityManager.createQuery(str, ProductCategory.class).setFlushMode(FlushModeType.COMMIT).setParameter("ids", productCategory.getTreePaths());
    if (count != null) {
      localTypedQuery.setMaxResults(count.intValue());
    }
    return localTypedQuery.getResultList();
  ...
  public List<ProductCategory> findChildren(ProductCategory productCategory, Integer count)
  {
    ...
    if (productCategory != null)
    {
      ...
      localTypedQuery = this.entityManager.createQuery(str, ProductCategory.class).setFlushMode(FlushModeType.COMMIT).setParameter("treePath", "%," + productCategory.getId() + "," + "%");
    }
    else
    {
      str = "select productCategory from ProductCategory productCategory order by productCategory.order asc";
      localTypedQuery = this.entityManager.createQuery(str, ProductCategory.class).setFlushMode(FlushModeType.COMMIT);
    }
    if (count != null) {
      localTypedQuery.setMaxResults(count.intValue());
    }
  ...
  public void remove(ProductCategory productCategory)
  {
    if (productCategory != null)
    {
      ...
      this.entityManager.createQuery(localStringBuffer.toString()).setFlushMode(FlushModeType.COMMIT).setParameter("productCategory", productCategory).executeUpdate();
 ...
 * Qualified Name:     com.tcop.dao.impl.ProductCategoryDaoImpl
