...
import com.tcop.dao.ArticleCategoryDao;
  ...
  implements ArticleCategoryDao
{
  public List<ArticleCategory> findRoots(Integer count)
  {
    ...
    TypedQuery localTypedQuery = this.entityManager.createQuery(str, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT);
    if (count != null) {
  ...
  public List<ArticleCategory> findParents(ArticleCategory articleCategory, Integer count)
  {
    ...
    TypedQuery localTypedQuery = this.entityManager.createQuery(str, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT).setParameter("ids", articleCategory.getTreePaths());
    if (count != null) {
      localTypedQuery.setMaxResults(count.intValue());
    }
    return localTypedQuery.getResultList();
  ...
  public List<ArticleCategory> findChildren(ArticleCategory articleCategory, Integer count)
  {
    ...
    if (articleCategory != null)
    {
      ...
      localTypedQuery = this.entityManager.createQuery(str, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT).setParameter("treePath", "%," + articleCategory.getId() + "," + "%");
    }
    else
    {
      str = "select articleCategory from ArticleCategory articleCategory order by articleCategory.order asc";
      localTypedQuery = this.entityManager.createQuery(str, ArticleCategory.class).setFlushMode(FlushModeType.COMMIT);
    }
    if (count != null) {
      localTypedQuery.setMaxResults(count.intValue());
    }
 ...
 * Qualified Name:     com.tcop.dao.impl.ArticleCategoryDaoImpl
