...
public class AttributeDaoImpl
  extends BaseDaoImpl<Attribute, Long>
  implements AttributeDao
{
  public void persist(Attribute attribute)
  {
    ...
    List localList = this.entityManager.createQuery(str, Integer.class).setFlushMode(FlushModeType.COMMIT).setParameter("productCategory", attribute.getProductCategory()).getResultList();
    for (int i = 0; i < 20; i++) {
  ...
  public void remove(Attribute attribute)
  {
    if (attribute != null)
    {
      ...
      this.entityManager.createQuery(str2).setFlushMode(FlushModeType.COMMIT).setParameter("productCategory", attribute.getProductCategory()).executeUpdate();
 ...
 * Qualified Name:     com.tcop.dao.impl.AttributeDaoImpl
